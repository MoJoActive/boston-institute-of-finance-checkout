...
function mergeCustomFields(
  lineItems: PhysicalItem[] | DigitalItem[] | undefined,
  productJson: any,
) {
  lineItems?.forEach((item: any) => {
    const product = productJson.data.site.products.edges.find(
      (o: any) => o.node.entityId === item.productId,
    );

    if (product) {
      item.customFields = product.node.customFields.edges.map((o: any) => ({
        name: o.node.name,
        value: o.node.value,
      }));
    }
...
> {
  ...
  async componentDidMount(): Promise<void> {
    ...
    try {
      const { data } = await loadCheckout(checkoutId, {
        params: {
          include: [
            ...
            'cart.lineItems.physicalItems.customFields',
            'cart.lineItems.digitalItems.customFields',
      ...
      const productsIds = [
        ...(cart?.lineItems.physicalItems.map((o) => o.productId) || []),
        ...(cart?.lineItems.digitalItems.map((o) => o.productId) || []),
      ];
      const productData = await fetch('/graphql', {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${(window as any).storefrontToken}`,
        },
        body: JSON.stringify({
          query: `query {
                site {
      ...
      mergeCustomFields(cart?.lineItems.physicalItems, productJson);
      mergeCustomFields(cart?.lineItems.digitalItems, productJson);
  ...
  private renderShippingStep(step: CheckoutStepStatus): ReactNode {
    const { hasCartChanged, cart, consignments = [], billingAddress } = this.props;
  ...
  private navigateToStep(type: CheckoutStepType, options?: { isDefault?: boolean }): void {
    ...
    if (options && options.isDefault) {
      this.setState({ defaultStepType: CheckoutStepType.Shipping });
    } else {
